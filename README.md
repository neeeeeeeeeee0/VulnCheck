...
- <b>Сканирование портов (Port Scanning)</b>
  ...
  - •  Обнаружение узлов (Host Discovery):
...
•  Баннер-граббинг (Banner Grabbing):
...
•  Основные библиотеки:
  ...
  •  ipaddress: (Если используется) для удобной работы с IP-адресами и сетями (например, парсинг CIDR-нотаций).
  •  argparse: (Если используется) для парсинга аргументов командной строки.
